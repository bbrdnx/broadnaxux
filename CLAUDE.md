...
## Deploy
**`git push` alone does NOT deploy the site.** There is no CI that deploys on push. Pushing to GitHub only updates the repo; the live site on Cloudflare changes only when the Wrangler commands below are run. A new/edited case study that is committed but still 404s on `barbarabroadnax.com` almost always means the D1 migration was not applied and/or the Worker was not deployed. Both steps are separate from git and must be run explicitly (this was the June 2026 "deployed the case study but it's not showing up" issue, where only `git push` had run; the unlisted Premium Search Widget study lives in migration `0015_seed_psw_case_study.sql`).
