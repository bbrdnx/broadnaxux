...
function cineLogoChip(company: ResolvedCompany | null, fallbackName: string): string {
  ...
  const brand = attrEscape(company?.brand ?? '#0D1B1E');
...
function siteHeaderStyles(): string {
  return `
    ...
    .sn-toggle:hover,.sn-drop.open .sn-toggle{color:#0D1B1E;}
    .sn-toggle:focus-visible{outline:2px solid #E2403E;outline-offset:3px;border-radius:2px;}
    ...
    .sn-menu{position:absolute;top:calc(100% + 10px);left:0;min-width:280px;padding:7px;background:rgba(251,254,249,0.98);backdrop-filter:blur(14px) saturate(1.1);-webkit-backdrop-filter:blur(14px) saturate(1.1);border:1px solid rgba(13,27,30,0.10);border-radius:8px;box-shadow:0 30px 70px rgba(13,27,30,.16),0 4px 10px rgba(13,27,30,.07);opacity:0;transform:translateY(-6px) scale(0.985);transform-origin:top left;pointer-events:none;transition:opacity 0.24s cubic-bezier(0.16,1,0.3,1),transform 0.24s cubic-bezier(0.16,1,0.3,1);z-index:120;}
    ...
    .sn-menu a:hover,.sn-menu a:focus-visible{background:#F0F3EE;outline:none;}
    ...
    @media (max-width:768px){
      ...
      .sn-bottom{display:block;position:fixed;bottom:0;left:0;right:0;z-index:100;background:rgba(251,254,249,0.96);backdrop-filter:blur(14px) saturate(1.1);-webkit-backdrop-filter:blur(14px) saturate(1.1);border-top:1px solid rgba(13,27,30,0.10);font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif;}
      ...
      .sn-bottom-link:hover{color:#0D1B1E;}
...
:root {
  ...
  --accent:       #E2403E;
  --accent-deep:  #C0302E;
  --accent-wash:  rgba(226,64,62,0.10);
  --accent-line:  rgba(226,64,62,0.32);

  --rule:         rgba(13,27,30,0.10);
  --rule-2:       rgba(13,27,30,0.18);
  --rule-dark:    rgba(251,254,249,0.12);

  --r-xs:2px; --r-sm:4px; --r-md:8px; --r-lg:14px; --r-xl:20px; --r-pill:999px;

  --sh-sm: 0 1px 2px rgba(13,27,30,.06), 0 1px 1px rgba(13,27,30,.04);
  --sh-md: 0 10px 30px rgba(13,27,30,.10), 0 2px 6px rgba(13,27,30,.05);
  --sh-lg: 0 30px 70px rgba(13,27,30,.16), 0 4px 10px rgba(13,27,30,.07);
...
.side-head .r {
  ...
  color: rgba(251,254,249,.7); max-width: 38ch; margin: 0;
}
.side-grid {
...
.spc-mark {
  ...
  background: linear-gradient(135deg, #0D1B1E, var(--ink-dark));
  border: 1px solid var(--rule-dark);
...
.spc-desc {
  ...
  color: rgba(251,254,249,.6); margin: 0;
}
.spc .arr {
  color: rgba(251,254,249,.4); font-size: 16px;
...
<head>
  ...
  <style>
    body { font-family: 'Inter', -apple-system, sans-serif; background: #FBFEF9; color: #0D1B1E; min-height: 100vh; display: grid; place-items: center; margin: 0; padding: 2rem; text-align: center; }
    a { color: #E2403E; }
  </style>
</head>
<body>
...
<style>body{font-family:'Inter',-apple-system,sans-serif;background:#FBFEF9;color:#0D1B1E;min-height:100vh;display:grid;place-items:center;margin:0;padding:2rem;text-align:center;}a{color:#E2403E;}</style></head>
...
<meta name="robots" content="noindex,nofollow">
<style>body{font-family:'Inter',-apple-system,sans-serif;background:#FBFEF9;color:#0D1B1E;min-height:100vh;display:grid;place-items:center;margin:0;padding:2rem;text-align:center;}a{color:#E2403E;}
.email-copy{background:none;border:none;cursor:pointer;font-family:inherit;font-size:inherit;color:#E2403E;padding:0;display:inline-flex;align-items:center;gap:0.35rem;transition:color 0.2s;}
.email-copy:hover{color:#0D1B1E;}.email-copy.copied{color:#16a34a;}</style></head>
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    body{font-family:'Inter',-apple-system,sans-serif;background:#FBFEF9;color:#0D1B1E;min-height:100vh;display:grid;place-items:center;-webkit-font-smoothing:antialiased;}
    .card{background:#FBFEF9;border:1px solid rgba(13,27,30,0.1);padding:2.5rem;border-radius:8px;width:min(420px, 92vw);box-shadow:0 12px 48px rgba(13,27,30,0.04);}
    ...
    input{width:100%;padding:0.75rem 0.85rem;border:1px solid rgba(13,27,30,0.15);border-radius:4px;background:#FBFEF9;color:#0D1B1E;font-family:inherit;font-size:0.95rem;}
    input:focus{outline:none;border-color:#E2403E;box-shadow:0 0 0 3px rgba(226,64,62,0.15);}
    button{margin-top:1rem;width:100%;padding:0.85rem;background:#E2403E;color:#FBFEF9;border:0;border-radius:4px;font-family:inherit;font-weight:600;font-size:0.92rem;cursor:pointer;letter-spacing:0.02em;}
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    :root{
      --white:#FBFEF9; --ink:#0D1B1E; --purple:#E2403E;
      --muted:#8B7F6A; --rule:rgba(13,27,30,0.1);
    ...
    .record-row:hover{background:rgba(226,64,62,0.05);}
    ...
    .record-logo{display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;padding:4px;border-radius:6px;background:color-mix(in srgb, var(--brand,#0D1B1E) 12%, #FBFEF9);border:1px solid color-mix(in srgb, var(--brand,#0D1B1E) 22%, transparent);flex-shrink:0;}
    ...
    .record-draft{display:inline-block;font-size:0.55rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:0.15rem 0.5rem;background:rgba(247,179,43,0.15);color:#a17500;border-radius:100px;margin-left:0.5rem;vertical-align:middle;}
