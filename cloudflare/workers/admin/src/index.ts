...
const SHARED_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, system-ui, sans-serif; background: #0b0b0f; color: #e0e0e8; min-height: 100vh; padding: 0; margin: 0; -webkit-font-smoothing: antialiased; }
  a { color: #c084fc; text-decoration: none; }
  a:hover { text-decoration: underline; }
  .topbar { background: #15151f; border-bottom: 1px solid #20202c; padding: 0.85rem 2rem; display: flex; align-items: center; gap: 2rem; position: sticky; top: 0; z-index: 100; }
  .topbar h1 { font-size: 0.85rem; font-weight: 700; color: #c084fc; letter-spacing: 0.08em; text-transform: uppercase; margin: 0; }
  .topbar nav { display: flex; gap: 1.25rem; flex: 1; }
  .topbar nav a { font-size: 0.78rem; color: #8a8a9a; letter-spacing: 0.04em; }
  .topbar nav a:hover, .topbar nav a.active { color: #e0e0e8; text-decoration: none; }
  .topbar form { margin: 0; }
  .topbar button { background: transparent; color: #8a8a9a; border: 1px solid #2a2a35; padding: 0.4rem 0.8rem; border-radius: 4px; font-size: 0.75rem; font-family: inherit; cursor: pointer; }
  .topbar button:hover { color: #c084fc; border-color: #c084fc; }
  .wrap { max-width: 1100px; margin: 0 auto; padding: 2.5rem 2rem 4rem; }
  .wrap h2 { font-size: 1.4rem; font-weight: 500; color: #c084fc; letter-spacing: 0.04em; margin: 0 0 0.4rem; }
  .wrap .sub { color: #8a8a9a; font-size: 0.85rem; margin-bottom: 2rem; }
  .toolbar { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem; flex-wrap: wrap; }
  .btn { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.55rem 1rem; background: #c084fc; color: #0b0b0f; font-weight: 600; border-radius: 4px; border: 0; cursor: pointer; font-family: inherit; font-size: 0.85rem; text-decoration: none; }
  .btn:hover { filter: brightness(1.1); text-decoration: none; }
  .btn.secondary { background: transparent; color: #e0e0e8; border: 1px solid #2a2a35; }
  ...
  .badge.draft { background: rgba(255, 200, 100, 0.12); color: #ffc864; border: 1px solid rgba(255, 200, 100, 0.25); }
  .badge.hidden { background: rgba(140, 140, 160, 0.12); color: #8a8a9a; border: 1px solid rgba(140, 140, 160, 0.25); }

  table.list { width: 100%; border-collapse: collapse; background: #15151f; border-radius: 8px; overflow: hidden; border: 1px solid #20202c; }
  table.list th, table.list td { padding: 0.75rem 1rem; text-align: left; border-bottom: 1px solid #20202c; vertical-align: middle; font-size: 0.85rem; }
  table.list th { font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.1em; color: #7c6d90; font-weight: 700; background: #1a1a26; }
  ...
  table.list .small { color: #7c6d90; font-size: 0.78rem; }

  .form-grid { display: grid; gap: 1.1rem; }
  .field { display: flex; flex-direction: column; gap: 0.4rem; }
  .field label { font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.1em; color: #7c6d90; font-weight: 700; }
  .field input, .field textarea, .field select { background: #0b0b0f; border: 1px solid #2a2a35; color: #e0e0e8; padding: 0.65rem 0.8rem; border-radius: 4px; font-family: inherit; font-size: 0.9rem; width: 100%; }
  ...
  .field input:focus, .field textarea:focus, .field select:focus { outline: none; border-color: #c084fc; }
  .field .hint { font-size: 0.72rem; color: #7c6d90; }

  .row2 { display: grid; grid-template-columns: 1fr 1fr; gap: 1.1rem; }
  @media (max-width: 720px) { .row2 { grid-template-columns: 1fr; } }

  .toast { position: fixed; bottom: 1.5rem; right: 1.5rem; background: #15151f; color: #e0e0e8; padding: 0.7rem 1.1rem; border: 1px solid #2a2a35; border-radius: 6px; font-size: 0.85rem; box-shadow: 0 8px 32px rgba(0,0,0,0.4); transition: opacity 0.4s; }
...
async function dashboardPage(env: Env): Promise<Response> {
  ...
  const body = `
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
      <a href="/case-studies" style="background: #15151f; border: 1px solid #20202c; padding: 1.25rem; border-radius: 8px; text-decoration: none;">
      ...
      <a href="/content" style="background: #15151f; border: 1px solid #20202c; padding: 1.25rem; border-radius: 8px; text-decoration: none;">
        ...
        <div style="font-size: 1.6rem; font-weight: 700; color: #e0e0e8; margin: 0.4rem 0;">${cn?.n ?? 0}</div>
        <div style="color: #8a8a9a; font-size: 0.78rem;">homepage copy + ticker</div>
      </a>
      <a href="/share-links" style="background: #15151f; border: 1px solid #20202c; padding: 1.25rem; border-radius: 8px; text-decoration: none;">
        <div style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.1em; color: #7c6d90;">Share Links</div>
        <div style="font-size: 1.6rem; font-weight: 700; color: #e0e0e8; margin: 0.4rem 0;">${sl?.total ?? 0}</div>
        <div style="color: #8a8a9a; font-size: 0.78rem;">${sl?.active ?? 0} active</div>
      </a>
      <a href="https://barbarabroadnax.com/" target="_blank" rel="noopener" style="background: #15151f; border: 1px solid #20202c; padding: 1.25rem; border-radius: 8px; text-decoration: none;">
        <div style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.1em; color: #7c6d90;">Public Site</div>
        <div style="font-size: 1rem; font-weight: 600; color: #c084fc; margin: 0.4rem 0;">view live →</div>
        <div style="color: #8a8a9a; font-size: 0.78rem;">opens in new tab</div>
      </a>
    </div>

    <div style="background: #15151f; border: 1px solid #20202c; border-radius: 8px; padding: 1.25rem 1.5rem;">
      <div style="font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.1em; color: #7c6d90; margin-bottom: 0.5rem;">Quick actions</div>
...
function caseStudyListRow(r: CaseStudyRow, idx: number, total: number): string {
  ...
  return `        <tr>
          ...
          <td>
            <a href="/case-studies/${attrEscape(r.id)}" style="font-weight: 600; color: #e0e0e8;">${htmlEscape(r.title)}</a>
...
async function editCaseStudyPage(env: Env, id: string | null, url: URL): Promise<Response> {
  ...
  const body = `
    <div class="toolbar">
      <a href="/case-studies" class="small" style="color: #8a8a9a;">← All case studies</a>
    </div>
    ...
    <form method="POST" action="${isNew ? '/case-studies/new' : '/case-studies/' + attrEscape(row!.id)}" id="csForm" class="form-grid">
      ...
      <div class="field">
        ...
        <div id="editor" style="height: 540px; border: 1px solid #2a2a35; border-radius: 4px; overflow: hidden;"></div>
...
async function editCaseStudyVersionPage(
  ...
  const inheritsBadge = (val: string | null | undefined) => val == null
    ? '<span style="background:rgba(124,109,144,0.15);color:#7c6d90;font-size:0.55rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:0.15rem 0.4rem;border-radius:100px;margin-left:0.4rem;">inherits</span>'
    : '<span style="background:rgba(192,132,252,0.12);color:#c084fc;font-size:0.55rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:0.15rem 0.4rem;border-radius:100px;margin-left:0.4rem;">override</span>';
  ...
  const body = `
    <div class="toolbar">
      <a href="/case-studies/${attrEscape(cs.id)}" class="small" style="color:#8a8a9a;">← ${htmlEscape(cs.title)}</a>
    </div>
    <h2>${isNew ? 'New version' : htmlEscape(row!.label)} <span style="color:#7c6d90;font-weight:400;">— variant of ${htmlEscape(cs.title)}</span></h2>
    ...
    ${isNew ? '' : `
    <div style="background:#15151f;border:1px solid #20202c;border-radius:6px;padding:0.85rem 1.1rem;margin-bottom:1.5rem;display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap;">
      <div style="flex:1;min-width:0;">
        <div style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;color:#7c6d90;margin-bottom:0.25rem;">Variant URL</div>
        <code style="color:#c084fc;font-size:0.82rem;word-break:break-all;">${htmlEscape(previewUrl)}</code>
      </div>
    ...
    <form method="POST" action="${isNew ? `/case-studies/${attrEscape(cs.id)}/versions/new` : `/case-studies/${attrEscape(cs.id)}/versions/${attrEscape(row!.id)}`}" id="vForm" class="form-grid">
      ...
      <div class="field">
        ...
        <div class="hint">Canonical: <code style="color:#7c6d90;">${htmlEscape((cs.subtitle ?? '').slice(0, 200) || '(empty)')}</code></div>
      </div>
      ...
      <div class="field">
        ...
        <div id="editor" style="height:480px;border:1px solid #2a2a35;border-radius:4px;overflow:hidden;"></div>
        <textarea id="body_html_textarea" name="body_html" style="display:none">${htmlEscape(row?.body_html ?? '')}</textarea>
...
function renderVersionsSection(csId: string, csTitle: string, versions: CaseStudyVersionRow[]): string {
  const rows = versions.map((v) => {
    ...
    const overrideLabel = overrides.length ? overrides.join(' · ') : '<span style="color:#7c6d90;">no overrides — pure inherit</span>';
    const updated = formatTimestamp(v.updated_at);
    const previewUrl = `https://barbarabroadnax.com/work/${csId}?v=${v.id}`;
    return `      <tr>
        <td><a href="/case-studies/${attrEscape(csId)}/versions/${attrEscape(v.id)}" style="font-weight:600;color:#e0e0e8;">${htmlEscape(v.label)}</a></td>
        <td class="small">${overrideLabel}</td>
        <td class="small" style="color:#7c6d90;">${htmlEscape(updated)}</td>
        <td>
          <div class="actions">
  ...
  const empty = versions.length === 0
    ? `<p class="sub" style="color:#7c6d90;margin:0.5rem 0 1rem;">No versions yet. Create one to send a tailored cut of <em>${htmlEscape(csTitle)}</em> via a share-link without forking the canonical page.</p>`
    : '';

  return `
    <div style="margin-top:3rem;border-top:1px solid #20202c;padding-top:2rem;">
      <div class="toolbar" style="margin-bottom:0.75rem;">
        <h3 style="margin:0;font-size:1rem;color:#e0e0e8;">Versions of this case study</h3>
    ...
    <table class="list">
      ...
      <tbody>
        ${rows.map((r) => {
          ...
          return `<tr>
            <td>
              <a href="/content/${attrEscape(r.key)}" style="font-weight: 600; color: #e0e0e8;">${htmlEscape(label)}</a>
...
async function editContentPage(env: Env, key: string, url: URL): Promise<Response> {
  ...
  const body = `
    <div class="toolbar">
      <a href="/content" class="small" style="color: #8a8a9a;">← All site content</a>
    </div>
...
<style>
  body{font-family:-apple-system,system-ui,sans-serif;background:#0b0b0f;color:#e0e0e8;min-height:100vh;display:grid;place-items:center;margin:0;}
  form{background:#15151f;padding:2rem;border-radius:8px;min-width:320px;box-shadow:0 10px 40px rgba(0,0,0,0.4);}
  h1{margin:0 0 .25rem;font-weight:500;font-size:1.15rem;color:#c084fc;letter-spacing:0.04em;}
  p.sub{margin:0 0 1.5rem;color:#7c6d90;font-size:.85rem;}
  label{font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:#7c6d90;display:block;margin-bottom:.5rem;}
  input{width:100%;padding:.75rem;border-radius:4px;border:1px solid #2a2a35;background:#0b0b0f;color:#e0e0e8;box-sizing:border-box;font-family:inherit;font-size:1rem;}
  input:focus{outline:none;border-color:#c084fc;}
  button{width:100%;margin-top:1rem;padding:.75rem;border:0;border-radius:4px;background:#c084fc;color:#0b0b0f;font-weight:600;cursor:pointer;font-size:.95rem;}
...
async function listShareLinksPage(env: Env, url: URL): Promise<Response> {
  ...
  try {
    ...
    const body = `
      ...
      <pre style="background:#15151f;border:1px solid #20202c;border-radius:6px;padding:1rem;color:#8eecb1;font-size:0.78rem;overflow:auto;">wrangler d1 execute broadnaxux-content --remote --file=cloudflare/migrations/0003_share_links.sql</pre>
  ...
  const body = `
    ...
    ${rows.length === 0 ? `
      <div style="background:#15151f;border:1px solid #20202c;border-radius:8px;padding:2.5rem;text-align:center;color:#8a8a9a;">
...
function shareLinkListRow(r: ShareLinkListRow): string {
  ...
  const pwBadge = r.password_hash
    ? '<span class="badge published" style="background: rgba(192,132,252,0.12); color:#c084fc; border-color: rgba(192,132,252,0.3); margin-left:0.4rem;">password</span>'
    : '';
  ...
  return `        <tr>
          <td>
            <a href="/share-links/${attrEscape(r.id)}" style="font-weight: 600; color: #e0e0e8;">${htmlEscape(r.name)}</a>${pwBadge}${expiredBadge}
            <div class="small">
              <a href="${attrEscape(publicUrl)}" target="_blank" rel="noopener" style="color: #c084fc;">${htmlEscape(displayPath)}</a>
...
async function editShareLinkPage(env: Env, id: string | null, url: URL): Promise<Response> {
  ...
  const fallbackBlock = !isNew && row!.slug
    ? `<div style="font-size:0.7rem;color:#7c6d90;margin-top:0.4rem;">also reachable as <code style="color:#7c6d90;">${htmlEscape(publicShareUrl(row!.token))}</code></div>`
    : '';
  const publicUrlBlock = isNew ? '' : `
    <div style="background:#15151f;border:1px solid #20202c;border-radius:6px;padding:1rem 1.1rem;margin-bottom:1.5rem;display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap;">
      <div style="flex:1;min-width:0;">
        <div style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;color:#7c6d90;margin-bottom:0.25rem;">Public URL</div>
        <code style="color:#c084fc;font-size:0.85rem;word-break:break-all;">${htmlEscape(primaryUrl)}</code>${fallbackBlock}
      </div>
  ...
  const resumeBlock = row?.resume_file_key ? `
    <div style="background:#0b0b0f;border:1px solid #2a2a35;border-radius:4px;padding:0.55rem 0.8rem;margin-bottom:0.5rem;display:flex;align-items:center;gap:0.5rem;font-size:0.85rem;">
  ...
  const body = `
    <div class="toolbar">
      <a href="/share-links" class="small" style="color: #8a8a9a;">← All share-links</a>
    </div>
    ...
    <form method="POST" action="${isNew ? '/share-links/new' : '/share-links/' + attrEscape(row!.id)}" enctype="multipart/form-data" id="slForm" class="form-grid">
      ...
      <div class="field">
        ...
        <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
          <span style="color:#7c6d90;font-size:0.85rem;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;white-space:nowrap;">barbarabroadnax.com/r/</span>
      ...
      <div class="field">
        ...
        <div style="display:grid;grid-template-columns: 1fr 1fr; gap: 1rem;">
          <div style="background:#0b0b0f;border:1px solid #2a2a35;border-radius:4px;padding:0.5rem;">
            <div style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;color:#7c6d90;padding:0.3rem 0.5rem 0.5rem;">Available</div>
            <div id="slAvailable"></div>
          </div>
          <div style="background:#0b0b0f;border:1px solid #2a2a35;border-radius:4px;padding:0.5rem;">
            <div style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;color:#7c6d90;padding:0.3rem 0.5rem 0.5rem;">In this share-link <span id="slCountBadge" style="color:#c084fc;"></span></div>
  ...
  const composerScript = `<script>
    (function () {
      ...
      function statusBadge(s) {
        ...
        return '<span style="background:rgba(255,200,100,0.12);color:#ffc864;font-size:0.55rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:0.15rem 0.4rem;border-radius:100px;border:1px solid rgba(255,200,100,0.25);margin-left:0.4rem;">' + s + '</span>';
      }
      function escAttr(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
      ...
      function render() {
        ...
        all.filter(c => !sel.has(c.id)).forEach(c => {
          ...
          row.innerHTML =
            ...
            '<div style="flex:1;min-width:0;"><div style="font-size:0.85rem;font-weight:600;color:#e0e0e8;">' + escAttr(c.title) + statusBadge(c.status) + '</div><div style="font-size:0.7rem;color:#7c6d90;">' + escAttr(c.company) + '</div></div>';
          avEl.appendChild(row);
        });
        if (avEl.children.length === 0) {
          avEl.innerHTML = '<div style="font-size:0.78rem;color:#7c6d90;padding:0.6rem 0.5rem;">All case studies are in this share-link.</div>';
        }
        ...
        selected.forEach((id, i) => {
          ...
          if (!c) return;
          const row = document.createElement('div');
          row.style.cssText = rowStyle() + 'background:#15151f;';
          row.draggable = true;
          row.dataset.id = c.id;
          ...
          if (versions.length > 0) {
            ...
            const optionsHtml = ['<option value="">Canonical (default)</option>']
              ...
              .join('');
            versionSelectHtml =
              '<select data-version-for="' + escAttr(c.id) + '" style="background:#0b0b0f;border:1px solid #2a2a35;color:#e0e0e8;font-size:0.65rem;border-radius:3px;padding:0.1rem 0.3rem;margin-left:0.4rem;max-width:11rem;">' +
              optionsHtml + '</select>';
          }

          row.innerHTML =
            '<span class="sl-drag" title="Drag to reorder" style="cursor:grab;color:#7c6d90;font-size:0.95rem;line-height:1;user-select:none;padding:0 0.15rem;">⋮⋮</span>' +
            '<span style="font-size:0.62rem;color:#7c6d90;font-weight:700;min-width:1.4rem;text-align:right;">' + (i+1) + '.</span>' +
            '<div style="flex:1;min-width:0;"><div style="font-size:0.85rem;font-weight:600;color:#e0e0e8;">' + escAttr(c.title) + statusBadge(c.status) + '</div><div style="font-size:0.7rem;color:#7c6d90;display:flex;align-items:center;flex-wrap:wrap;">' + escAttr(c.company) + versionSelectHtml + '</div></div>' +
        ...
        if (seEl.children.length === 0) {
          seEl.innerHTML = '<div style="font-size:0.78rem;color:#7c6d90;padding:0.6rem 0.5rem;">Pick case studies from the left.</div>';
        }
      ...
      seEl.addEventListener('dragover', (e) => {
        ...
        row.style.boxShadow = above ? '0 -2px 0 0 #c084fc' : '0 2px 0 0 #c084fc';
      });
...
function eventBadge(event: string): string {
  const colors: Record<string, [string, string]> = {
    open: ['rgba(192,132,252,0.15)', '#c084fc'],
    card_click: ['rgba(142,236,177,0.12)', '#8eecb1'],
    resume_download: ['rgba(255,200,100,0.12)', '#ffc864'],
    unlock_failed: ['rgba(255,107,107,0.12)', '#ff6b6b'],
  };
  const [bg, fg] = colors[event] ?? ['rgba(124,109,144,0.12)', '#8a8a9a'];
...
async function analyticsShareLinkPage(env: Env, id: string, url: URL): Promise<Response> {
  ...
  function tile(label: string, value: number, color: string): string {
    return `<div style="background:#15151f;border:1px solid #20202c;border-radius:8px;padding:1rem 1.1rem;">
      <div style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;color:#7c6d90;margin-bottom:0.4rem;">${label}</div>
  ...
  const bars = days.map((d) => {
    ...
    return `<div style="flex:1;display:flex;flex-direction:column;align-items:stretch;justify-content:flex-end;height:140px;min-width:8px;" title="${attrEscape(tooltip)}">
      <div style="width:100%;background:${total > 0 ? '#c084fc' : '#20202c'};height:${total > 0 ? Math.max(heightPct, 4) : 2}%;border-radius:2px 2px 0 0;"></div>
    </div>`;
  }).join('');
  const xLabels = days.map((d, i) => {
    // Only show every 5th label to avoid clutter
    return i % 5 === 0 ? `<div style="flex:1;text-align:left;font-size:0.6rem;color:#7c6d90;font-variant-numeric:tabular-nums;min-width:8px;">${htmlEscape(d.slice(5))}</div>` : `<div style="flex:1;min-width:8px;"></div>`;
  }).join('');
  ...
  const csRows = (perCsRes.results ?? []).map((r) => {
    ...
    return `<tr>
      <td><div style="font-weight:600;color:#e0e0e8;">${htmlEscape(title)}</div><div class="small" style="color:#7c6d90;">${htmlEscape(company)} · <code>${htmlEscape(r.id)}</code></div></td>
      <td style="text-align:right;font-variant-numeric:tabular-nums;font-weight:700;color:#c084fc;font-size:1rem;">${r.n}</td>
    </tr>`;
  }).join('');
  ...
  const recentRows = (recentRes.results ?? []).map((r) => {
    ...
    const detail = r.case_study_id ? `<code style="color:#8a8a9a;">${htmlEscape(r.case_study_id)}</code>` : '<span style="color:#7c6d90;">—</span>';
    const ua = r.user_agent ? shortUA(r.user_agent) : '—';
    const ref = r.referrer ? ' ← ' + shortRef(r.referrer) : '';
    return `<tr>
      <td class="small" style="color:#7c6d90;font-variant-numeric:tabular-nums;white-space:nowrap;">${htmlEscape(ts)}Z</td>
      <td>${eventBadge(r.event)}</td>
      <td class="small">${detail}</td>
      <td class="small" style="color:#7c6d90;">${htmlEscape(ua + ref)}</td>
    </tr>`;
  }).join('');
  ...
  const body = `
    <div class="toolbar">
      <a href="/share-links" class="small" style="color:#8a8a9a;">← All share-links</a>
      <a href="/share-links/${attrEscape(id)}" class="small" style="color:#8a8a9a;">Edit this link</a>
    </div>
    <h2>${htmlEscape(link.name)} <span style="color:#7c6d90;font-weight:400;">— Analytics</span></h2>
    <p class="sub">
      ...
      <br><a href="${attrEscape(publicUrl)}" target="_blank" rel="noopener" style="color:#c084fc;">${htmlEscape(publicUrl)}</a>
    </p>

    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:1rem;margin-bottom:1.5rem;">
      ${tile('Opens', counts.open ?? 0, '#c084fc')}
      ${tile('Card clicks', counts.card_click ?? 0, '#8eecb1')}
      ${tile('Resume downloads', counts.resume_download ?? 0, '#ffc864')}
      ${tile('Unlock failures', counts.unlock_failed ?? 0, '#ff6b6b')}
    </div>

    <h3 style="margin-top:2rem;font-size:0.95rem;color:#e0e0e8;">Last 30 days · ${totalEvents} event${totalEvents === 1 ? '' : 's'} total</h3>
    <div style="background:#15151f;border:1px solid #20202c;border-radius:8px;padding:1rem;">
      ...
      <div class="small" style="color:#7c6d90;margin-top:0.5rem;">Hover a bar for the per-event breakdown.</div>
    </div>

    <h3 style="margin-top:2rem;font-size:0.95rem;color:#e0e0e8;">Card clicks by case study</h3>
    ${csRows ? `<table class="list">
      <thead><tr><th>Case study</th><th style="text-align:right;">Clicks</th></tr></thead>
      <tbody>${csRows}</tbody>
    </table>` : '<p class="sub" style="color:#7c6d90;">No card clicks yet.</p>'}

    <h3 style="margin-top:2rem;font-size:0.95rem;color:#e0e0e8;">Recent activity (last 50 events)</h3>
    ${recentRows ? `<table class="list">
      <thead><tr><th>When (UTC)</th><th>Event</th><th>Case study</th><th>Browser · referrer</th></tr></thead>
      <tbody>${recentRows}</tbody>
    </table>` : '<p class="sub" style="color:#7c6d90;">No events yet.</p>'}
  `;
